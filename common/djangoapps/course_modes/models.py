...
def get_course_prices(course, verified_only=False, currency=None):
    ...
    if verified_only:
        registration_price = CourseMode.min_course_price_for_verified_for_currency(
            course.id,
            settings.PAID_COURSE_REGISTRATION_CURRENCY[0] if currency is None else currency
        )
    else:
...
def format_course_price(price, currency=None):
    ...
    """
    symbol = '₹' if currency is not None else None
    currency_symbol = settings.PAID_COURSE_REGISTRATION_CURRENCY[1] if symbol is None else symbol
